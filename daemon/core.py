...
def _start_cli_background(*args, log_file=None):
    ...
    fh = open(log_file, "a") if log_file else subprocess.DEVNULL
    ...
    if log_file and fh != subprocess.DEVNULL:
        fh.close()
    return proc
...
def check_jobs():
    ...
    for entry in entries:
        ...
        # Process is dead — try to reap zombie
        rc = None
        try:
...
def cleanup_old_entries():
    ...
    if removed > 0:
        _save_registry(kept)
        logger.info(f"Cleaned up {removed} expired job entries")
